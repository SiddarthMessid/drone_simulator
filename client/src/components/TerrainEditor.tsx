...
export default function TerrainEditor() {
  ...
  const { getAllObstacles, clearObstacles } = useEnvironment();
  ...
  const handleTerrainToggle = () => {
    ...
    // If switching to mountain terrain and there are obstacles, show confirmation
    if (isFlat && obstacles.length > 0) {
      const confirmed = window.confirm(
        `Switching to Mountain Terrain will remove all ${obstacles.length} obstacle(s).\n\nDo you want to continue?`
      );
      ...
      // No obstacles or switching to flat terrain
      toggleFlat();
    }
  };
                ...
                <div style={{ marginBottom: "12px" }}>
                  ...
                  <Slider
                    ...
                    onValueChange={(values) =>
                      updateTerrainConfig({ maxHeight: values[0] })
                    }
                    min={20}
                    max={200}
                    step={5}
              ...
              <div style={{ marginBottom: "20px" }}>
                ...
                <div style={{ marginBottom: "12px" }}>
                  ...
                  <Slider
                    ...
                    onValueChange={(values) =>
                      updateTerrainConfig({ resolution: values[0] })
                    }
                    min={64}
                    max={512}
                    step={64}
                ...
                >
                  ...
                  <div style={{ marginBottom: "12px" }}>
                    ...
                    <Slider
                      value={[layer.scale]}
                      onValueChange={(values) =>
                        updateLayer(index, { scale: values[0] })
                      }
                      min={0.001}
                      max={0.1}
                      step={0.001}
                  ...
                  <div style={{ marginBottom: "12px" }}>
                    ...
                    <Slider
                      ...
                      onValueChange={(values) =>
                        updateLayer(index, { amplitude: values[0] })
                      }
                      min={0.1}
                      max={2.0}
                      step={0.1}
                  ...
                  <div style={{ marginBottom: "12px" }}>
                    ...
                    >
                      ...
                      <span style={{ color: "#3b82f6", fontWeight: "bold" }}>
                        {layer.persistence.toFixed(2)}
                      </span>
                    </div>
                    <Slider
                      value={[layer.persistence]}
                      onValueChange={(values) =>
                        updateLayer(index, { persistence: values[0] })
                      }
                      min={0.1}
                      max={0.9}
                      step={0.05}
                  ...
                  <div style={{ marginBottom: "12px" }}>
                    ...
                    <Slider
                      value={[layer.octaves]}
                      onValueChange={(values) =>
                        updateLayer(index, { octaves: values[0] })
                      }
                      min={1}
                      max={6}
                      step={1}
