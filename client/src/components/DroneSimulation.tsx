    ...
    <>
      ...
      <mesh
        ...
        onPointerDown={(e: any) => {
          ...
          const { mode, setStart, setStop, addScanPoint } =
            useMission.getState();
      ...
      {(() => {
        ...
        if (startPoint) {
          markers.push(
            <mesh
              key="mission_start"
              position={[startPoint.x, startPoint.y + 0.2, startPoint.z]}
            >
        ...
        if (stopPoint) {
          markers.push(
            <mesh
              key="mission_stop"
              position={[stopPoint.x, stopPoint.y + 0.2, stopPoint.z]}
            >
              <sphereGeometry args={[0.2, 12, 12]} />
        ...
        scanPoints.forEach((s) => {
          markers.push(
            <mesh
              key={s.id}
              position={[s.position.x, s.position.y + 0.2, s.position.z]}
            >
