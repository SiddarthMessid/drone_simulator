...
export type CameraMode = "follow" | "fpv" | "manual" | "topView";
...
interface CameraState {
  ...
  enterMissionPlanningMode: () => void;
  exitMissionPlanningMode: () => void;
}
...
export const useCamera = create<CameraState>((set, get) => ({
  ...
  setTopViewHeight: (height) => set({ topViewHeight: height }),
  enterMissionPlanningMode: () => {
    const currentMode = get().mode;
    set({ previousMode: currentMode, mode: "topView" });
  },
  exitMissionPlanningMode: () => {
    const prevMode = get().previousMode || "follow";
    set({ mode: prevMode, previousMode: null });
  },
}));
