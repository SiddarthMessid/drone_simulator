      ...
      >
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          {windSources.length > 0 && (
            <button onClick={clearAllWind} style={dangerButtonStyle}>
              Clear All
            </button>
          )}
        </div>
          ...
          >
            <div>
              ...
              <input
                ...
                value={newWind.name}
                onChange={(e) =>
                  setNewWind({ ...newWind, name: e.target.value })
                }
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                value={newWind.force}
                onChange={(e) =>
                  setNewWind({ ...newWind, force: Number(e.target.value) })
                }
                style={inputStyle}
                min="0"
                max="50"
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewWind({ ...newWind, radius: Number(e.target.value) })
                }
                style={inputStyle}
                min="5"
                max="100"
          ...
          <div style={{ marginBottom: "12px" }}>
            ...
            <select
              ...
              onChange={(e) =>
                setNewWind({
                  ...newWind,
                  type: e.target.value as "constant" | "variable",
                })
              }
              style={inputStyle}
            >
            ...
            >
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    <div>
                      Force: {source.force.toFixed(1)} m/s | Direction:{" "}
                      {((source.direction * 180) / Math.PI).toFixed(0)}°
                    </div>
                    <div>
                      Position: ({source.position.x}, {source.position.y},{" "}
                      {source.position.z}) | Radius: {source.radius}m
                    </div>
                    <div>
                      Type: {source.type}
                      {source.type === "variable"
                        ? ` (${source.variableParams.frequency}Hz, ±${source.variableParams.amplitude})`
                        : ""}
                    </div>
                  </div>
                </div>
      ...
      {windSources.length > 0 && (
        ...
        >
          <div style={{ marginBottom: "4px" }}>
            <strong>Wind System Status:</strong>{" "}
            {windSources.filter((s) => s.enabled).length} active source(s)
          </div>
          <div>
            Wind forces are calculated based on drone position relative to each
            wind source's area of effect.
          </div>
        </div>
      )}
    </div>
