...
export class DroneController {
  ...
  async takeoff(targetAltitude: number = 10): Promise<void> {
    console.log(`[TAKEOFF] Starting takeoff to ${targetAltitude}m`);
    console.log(`[TAKEOFF] Current state - autopilot: ${this.isAutopilot}, hasCommand: ${!!this.currentCommand}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(`[TAKEOFF] Command created and executing`);
    });
  }
  ...
  async hover(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const command: DroneCommand = {
        ...
        reject,
        startTime: Date.now(),
        timeout: 10000 // 10 second timeout to allow for stabilization
      };
   ...
   * NOTE: Position controller is currently broken, this just logs and resolves
   */
  async moveTo(targetPosition: THREE.Vector3, options: { speed?: number, timeout?: number } = {}): Promise<void> {
    console.warn(`moveTo() is not implemented - position controller needs to be fixed`);
  ...
  private checkCommandCompletion(): void {
    ...
    switch (this.currentCommand.type) {
      ...
      case 'land':
        ...
        const verticallyStable = Math.abs(droneStore.velocity.y) < 0.3; // Vertical speed < 0.3 m/s
        // Must reach altitude AND be stable
        isComplete = altitudeReached && verticallyStable;
        break;

      case 'hover':
        // Check if drone is actually stable (low velocity and near target)
        const hoverTime = Date.now() - this.currentCommand.startTime;
        const isStable = droneStore.velocity.length() < 0.5; // Speed less than 0.5 m/s
        ...
        const isNearTarget = hasPosition
          ...
          : true;
        // Complete after minimum 1 second AND drone is stable
    ...
    if (isComplete) {
      ...
      // After any command completes, clear targets and disable autopilot
      // This prevents commands from interfering with each other
      this.targets = {};
      this.isAutopilot = false;
    }
  }
